package com.example.xnewsme.ui.viewmodel
...
import com.example.xnewsme.model.NewsResponse
import com.example.xnewsme.repository.NewsRepository
        ...
        if (response.isSuccessful) {
            response.body()?.let { resultRespones ->
                if (searchNewsResponse == null || newSearchQuery != oldSearchQuery ) {
                   ...
                   searchNewsResponse = resultRespones
                } else {
                    searchNewsPage++
                ...
                return Resource.Success(searchNewsResponse ?: resultRespones)
            }
        }
        return Resource.Error(response.message())
    }
    ...
    private suspend fun headlinesInternet(countryCode: String) {
        ...
        try {
            if (internetConnection(this.getApplication())) {
                Log.d("HeadlinesFragment", "Internet connection available")
                val response = newsRepository.getHeadlines(countryCode, headlinesPage)
                Log.d("HeadlinesFragment", "API call made, response: $response")
                headlines.postValue(handleHeadlineResponse(response))
            } else {
                Log.d("HeadlinesFragment", "No internet connection")
                headlines.postValue(Resource.Error("No internet connection"))
            }
        } catch (t: Throwable) {
            Log.e("HeadlinesFragment", "Error occurred: ${t.message}", t)
            when (t) {
